...
export const updateUser = async (req, res, next) => {
    try {
        ...
        if (isUsernameTaken && req.params.id !== isUsernameTaken._id.toString()) res.status(400).json(`Username already taken`)
...
export const followOrUnfollowUser = async (req, res, next) => {
    try {
        ...
        if (follower.following.includes(req.params.id)) {
            ...
            const notification = new Notif({ userId: followed._id, value: `${follower.username} started following you.`, sourceId: follower._id, type: 'follow' })
            await Promise.all([
                follower.save(),
                followed.save(),
                notification.save()
